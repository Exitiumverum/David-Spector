...
הגינה עוצבה מחדש עם צמחייה ופינת ישיבה מזמינה.',
  1
FROM projects p WHERE p.slug = 'athens-26m';
...
המטבח והסלון קומפקטיים אך מוארים, והגינה – שבעבר הייתה רחבת בטון מוזנחת – הפכה למרחב ירוק עם טרסות, דק עץ וצבעוניות רגועה על הקירות שמוסיפה חמימות ורוך.',
  1
FROM projects p WHERE p.slug = 'athens-21m';
...
את הגינה תכננתי מחדש כחלל חיצוני מזמין, עם צמחייה טבעית ופינת ישיבה רגועה.',
  1
FROM projects p WHERE p.slug = 'kipsli-28m';

-- Athens Apartment Content
INSERT INTO project_content (project_id, section, content_hebrew, display_order)
SELECT 
  p.id,
  'project_description',
...
כל אזור בדירה קיבל אופי משלו תוך שמירה על אחידות, כולל מרפסת עם כיסא נדנדה מפנק.',
  1
FROM projects p WHERE p.slug = 'athens-apartment';
